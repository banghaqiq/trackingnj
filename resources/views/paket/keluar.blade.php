...
@section('title', __('paket.outgoing_packages'))

@section('content')
<div class="row mb-4">
    <div class="col">
        <h2><i class="bi bi-send"></i> {{ __('paket.outgoing_packages') }}</h2>
        <p class="text-muted">Paket dengan status: Diterima, Selesai, Dikembalikan, dan Salah Wilayah</p>
    </div>
</div>

<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('paket.keluar') }}" class="row g-3">
            <div class="col-md-6">
            ...
            <div class="col-md-2 d-flex align-items-end">
                ...
                <a href="{{ route('paket.keluar') }}" class="btn btn-secondary">
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-{{ 
                                    ...
                                    ($item->status->value === 'selesai' ? 'primary' : 
                            ...
                            <td>{{ $item->tanggal_diambil?->format('d/m/Y H:i') ?? '-' }}</td>
                            <td>
                                <div class="btn-group" role="group">
                                    ...
                                    @if(in_array(Auth::user()->role->value, ['keamanan', 'petugas_pos']) && 
                                        in_array($item->status->value, ['diambil', 'diterima']))
                                        <div class="btn-group" role="group">
                                            ...
                                            <ul class="dropdown-menu">
                                                @if($item->status->value !== 'diterima')
                                                    <li>
                                                        <a class="dropdown-item" href="#" 
                                                           onclick="updateStatus({{ $item->id }}, 'diterima'); return false;">
                                                ...
                                                <li>
                                                    <a class="dropdown-item text-danger" href="#" 
                                                       onclick="updateStatus({{ $item->id }}, 'salah_wilayah'); return false;">
                                                        <i class="bi bi-x-circle"></i> Salah Wilayah
                                                    </a>
                                                </li>
                                                @if($item->status->value === 'diterima')
                                                    <li>
                                                        <a class="dropdown-item text-primary" href="#" 
                                                           onclick="updateStatus({{ $item->id }}, 'selesai'); return false;">
...
<script>
    ...
    function updateStatus(paketId, status) {
        ...
        fetch(`/paket/${paketId}/status`, {
            ...
            body: JSON.stringify({ 
                status: status,
                catatan: catatan || 'Status diubah dari paket keluar'
            })
        })
